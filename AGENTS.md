...
- Signed-in users use personal packs for Match. On first login with an empty pool, Starter pack strats are auto-copied for all maps (`ensureFundamentalsSeeded` + `localStorage` flag `cs2-playbook-fundamentals-seeded:<userId>`). Guests instead toggle system packs (Starter on by default; Meme off; Advanced locked). Filter Match / Playbook by FACEIT level bands (All / 1–3 / 4–6 / 7–10).
