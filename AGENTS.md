...
- Signed-in users use personal packs for Match. On first login with an empty pool, Starter pack strats are auto-copied for all maps (`ensureFundamentalsSeeded` + `localStorage` flag `cs2-playbook-fundamentals-seeded:<userId>`). Guests instead toggle system packs (Starter on by default; Meme off; Advanced locked).
