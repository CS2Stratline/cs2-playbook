...
- Each strat has `level` 1–10 (FACEIT-style execution difficulty). Icons live in `FaceitLevelIcon`; colors/Elo brackets in `src/lib/faceitLevels.ts`. After changing catalog levels, re-seed Supabase (`npm run seed:supabase`) once migration `005_strat_level.sql` is applied.
- Routing uses `HashRouter` (URLs look like `/#/...`) because the production target is GitHub Pages under a base path.
