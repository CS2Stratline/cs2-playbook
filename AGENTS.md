...
- Signed-in users use personal packs for Match. On first login with an empty pool, Starter Pack strats are auto-copied for all maps (`ensureStarterPackSeeded` + `localStorage` flag `cs2-playbook-fundamentals-seeded:<userId>`). Guests instead toggle system packs (Starter Pack on by default; Meme off; Advanced locked).
