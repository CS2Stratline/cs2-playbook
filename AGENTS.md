...
- Signed-in (permanent) users use a personal **My pool** for Match. On first login with an empty pool, Fundamentals are auto-copied for all maps (`ensureFundamentalsSeeded` + `localStorage` flag `cs2-playbook-fundamentals-seeded:<userId>`). Anonymous guests instead toggle system packs. Advanced (pro) packs are hidden in the UI until premium.
