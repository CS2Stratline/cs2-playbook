...
/**
 * Attach lineup links to the task line they belong to (by landing phrase).
...
export function attachLinksToTasks(
  ...
  const assignments: { link: StratLink; taskIndex: number; score: number }[] = [];

  for (const link of links) {
    let bestIdx = -1;
    let bestScore = 0;
    for (let i = 0; i < tasks.length; i++) {
      const score = linkTaskMatchScore(tasks[i], link);
      if (score > bestScore) {
        bestScore = score;
        bestIdx = i;
      }
    ...
    if (bestIdx >= 0 && bestScore > 0) {
      assignments.push({ link, taskIndex: bestIdx, score: bestScore });
    }
  }

  // Prefer stronger matches when multiple links land on the same step.
  assignments.sort((a, b) => b.score - a.score);

  const used = new Set<StratLink>();
  const rows = tasks.map((task) => ({ task, links: [] as StratLink[] }));
  for (const a of assignments) {
    if (used.has(a.link)) continue;
    used.add(a.link);
    rows[a.taskIndex].links.push(a.link);
  }

  const leftover = links.filter((l) => !used.has(l));
